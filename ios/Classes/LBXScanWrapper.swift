...
open class LBXScanWrapper: NSObject,AVCaptureMetadataOutputObjectsDelegate {
    ...
    var outPutSetting: AVCapturePhotoSettings = AVCapturePhotoSettings(format: [AVVideoCodecKey: AVVideoCodecType.jpeg])
    ...
    init(videoPreView: UIView,
        ...
        do {
            ...
            debugPrint("LBXScan AVCaptureDeviceInput() error (\(error))")
        }
        ...
        stillImageOutput.photoSettingsForSceneMonitoring = self.outPutSetting
        ...
        if device.isFocusPointOfInterestSupported && device.isFocusModeSupported(.continuousAutoFocus) {
            do {
                ...
                debugPrint("LBXScan device.lockForConfiguration() error (\(error))")
            }
        }
    }
...
//        }
        ...
        stillImageOutput.capturePhoto(with: self.outPutSetting, delegate: self)
...
//        })
    ...
    open func setTorch(torch: Bool) {
        ...
        do {
            ...
            debugPrint("LBXScan device.lockForConfiguration(): setTorch error (\(error))")
        }
    }
...
extension LBXScanWrapper {
    func zoomCamera(_ scale: CGFloat) {
        do {
            if let currentDevice = input?.device {
                try currentDevice.lockForConfiguration()
                let maxScale: CGFloat = 3.0
                let minScale: CGFloat = 1.0
                if scale > maxScale {
                    currentDevice.videoZoomFactor = maxScale
                } else if scale < minScale {
                    currentDevice.videoZoomFactor = minScale
                } else {
